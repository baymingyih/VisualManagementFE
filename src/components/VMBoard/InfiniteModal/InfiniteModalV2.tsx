import { Button, Card, Carousel, DatePicker, Form, Modal, Segmented, Spin, Typography } from "antd";
...
import { selectMetricDataById } from "../../../../redux/features/metricData/metricDataSlice";
...
function InfiniteModalV2() {
    const selectedFilterDate: string = useSelector(getFilterDate);
    const selectedMetric: IMetric | null = useSelector(getSelectedMetric);
    ...
    const [filterSelectedDate, setFilterSelectedDate] = useState<string>(selectedFilterDate);

    // this is required because of legacy implmeentation of using date string... should replace with numeric unix timestamp asap
    const [filterDateValue, setFilterDateValue] = useState<string>(selectedFilterDate);

    const prefetchData =
        useSelector((state: any) => selectMetricDataById(state, +selectedMetric!.metricId)) || [];
    ...
    const [data, setData] = useState<IMetricChartData[]>(prefetchData);
    ...
    const isMonthFilter = filterDataOption === FilterEnum.MONTH;
    const isWeekFilter = filterDataOption === FilterEnum.WEEK;
    const isYearFilter = filterDataOption === FilterEnum.YEAR;

    const onFilterDateChange = (date: moment.Moment | null) => {
        if (date) {
            setFilterSelectedDate(date.format("DD-MMM-YYYY"));
            setFilterDateValue(date.format("YYYY-MM-DD"));
        }
    };
    ...
    const { refetch: getMonthData, isLoading: isMonthLoading } = useQuery({
        queryKey: [
            ...
            {
                metricId: (selectedMetric && +selectedMetric?.metricId) || 0,
                date: selectedFilterDate,
            },
        ],
        queryFn: () =>
            getMetricDataByMonth(
                (selectedMetric && +selectedMetric?.metricId) || 0,
                selectedFilterDate
            ),
                    ...
                    title={
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            ...
                            <Button
                                ...
                                onClick={onCreateActionButtonClicked}>
                                Create Action
                            </Button>
                        </div>
                    }>
                            ...
                            <div style={{ display: "flex", flexFlow: "column nowrap", gap: 5 }}>
                                <div className="modal" style={{ alignSelf: "flex-end" }}>
                                    <DatePicker
                                        ...
                                        defaultValue={moment(filterSelectedDate)}
                                        showToday
                                        onChange={onFilterDateChange}
                                        suffixIcon={
                                            <Button icon={<CaretDownOutlined />} type="link" />
                                        }
...
export default InfiniteModalV2;
